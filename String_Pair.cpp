...
string convert(int num, map<int, string> conversion_hash){
    if(num == 0) return "";
...
string numberToWords(int num, map<int, string> conversion_hash) {
    string ret = "";
    int i = 0;
    if(num == 0) return "zero";
    string itm = convert(num, conversion_hash);
    ret += itm;
    if(ret[ret.size()-1] == ' ') {
...
int main(){
    ...
    for(int i = 0; i < n; i++){
        cin>>rec[i];
        string str = numberToWords(rec[i], conversion_hash);
        for(char x : str){
    ...
    cout<<d<<"\n";
    cout<<numberToWords(solve(rec, d, n), conversion_hash);
    return 0;
}
