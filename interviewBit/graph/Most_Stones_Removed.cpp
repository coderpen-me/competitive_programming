...
int find(int x){
    if (!f.count(x)) f[x] = x, islands++, b1("INcrement island");
    if (x != f[x]) f[x] = find(f[x]), b2("Find and f[x] is now", f[x]);
    return f[x];
}

void unify(int x, int y){
    a3("Called unify with", x, y);
    nl;
    x = find(x), y = find(y);
    a3("found unify as", x, y);
    nl;
    if (x != y) f[x] = y, islands--, b1("decrement island");
}
