...
void recurr(int l, int rem, int rev, int n, vs &ret, string &temp, map<string, int> &bag)
{
    if (rev > rem || rev > n || rem > n)
        return;
    if (l == 2 * n && rev - rem == 0 && bag[temp] == 0)
    {
        ret.push_back(temp);
        bag[temp] = 1;
        //b3(temp, rem, rev);
    ...
    {
        temp.push_back('(');
        recurr(l + 1, rem + 1, rev, n, ret, temp, bag);
        temp.pop_back();
    }
    if (rev < n)
    {
        temp.push_back(')');
        recurr(l + 1, rem, rev + 1, n, ret, temp, bag);
        temp.pop_back();
    }
}
